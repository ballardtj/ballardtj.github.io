...
layout: single
classes: wide
title: "Non-Centered Parameterisation in Hierarchical Bayesian Models: The Case of Double Bounded Parameters"
date: 2024-10-01
categories:
  - Blog
tags:
...
I wrote a post a few weeks about about using non-centered parameters in hierarchical Bayesian models. If you haven't read it, you may want to do so before reading this one. Check it out here. In the earlier post, I wrote about how even though it's not always obvious how to apply non-centered parameterisation to bounded or multivariate distributions, it's actually straightforward. The key is to apply a second transformation after the inverse-z-transform that imposes the appropriate constraints. I gave examples of how to do this with parameters that have a single truncation point (e.g., standard deviation parameters that are constrained to be greater than zero).

What about parameters that are double bounded? These are parameters that have both upper and lower bounds (e.g., probability parameters constrained to between 0 and 1). 
...
```{r simulate-data-1,messages=FALSE,warnings=FALSE}
...
X1 <- rtruncnorm(N, mean = 0, sd = 1)
X2 <- rtruncnorm(N, mean = 0, sd = 1)

# Population parameter values
prob_shape1 <- 4   # beta0 population mean
prob_shape2 <- 2   
mean1 <- 1
sd1 <- 1.5
mean2 <- 3
...
# Simulate variability in mixture probability parameter across participants
prob <- rbeta(n = J, shape1 = prob_shape1, shape2 = prob_shape2)

# Generate response variable
Y <- rnorm(N,mean=mean1,sd=sd1) * rep(prob,each=n_j) +
  rnorm(N,mean=mean2,sd=sd2) * (1-rep(prob,each=n_j)) 
...
parameters {
  real<lower=0> prob_shape1;          // Intercept population mean
  real<lower=0> prob_shape2;          // Effect of X1 population mean
  real mean1;            // Intercept population mean
  real mean2;            // Effect of X1 population mean
  real<lower=0> sd1;            // Intercept population mean
  real<lower=0> sd2;            // Effect of X1 population mean
  vector<lower=0>[J] prob;          // Participant intercepts
}

model {
  // Population-level Priors
  prob_shape1 ~ normal(0, 5);
  prob_shape2 ~ normal(0, 5);
  mean1 ~ normal(0, 5);
  mean2 ~ normal(0, 5);
  sd1 ~ normal(0, 5);
  sd2 ~ normal(0, 5);
  ...
  prob ~ beta(prob_shape1,prob_shape2);

  // Likelihood
  target += log_mix(prob[participant_id], 
               normal_lpdf(Y | mean1, sigma1),
               normal_lpdf(Y | mean2, sigma2));
